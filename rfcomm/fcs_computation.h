...
public:
/* FCS on 2 bytes */
inline u8 __fcs(u8 *data)
{
	return 0xff - __crc(data);
}

/* FCS on 3 bytes */
inline u8 __fcs2(u8 *data)
{
	return 0xff - rfcomm_crc_table[__crc(data) ^ data[2]];
}

/* Check FCS */
inline int __check_fcs(u8 *data, int type, u8 fcs)
{
	u8 f = __crc(data);
...
static inline u8 __fcs(u8 *data){
	return fcs_compute.__fcs(data);
}
static inline u8 __fcs2(u8 *data){
	return fcs_compute.__fcs2(data);
}
static inline int __check_fcs(u8 *data, int type, u8 fcs){
	return fcs_compute.__check_fcs(data, type, fcs);
}
