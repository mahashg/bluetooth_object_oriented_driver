...
void HIDP_CORE::hidp_copy_session(struct hidp_session *session, struct hidp_conninfo *ci)
{
...
int HIDP_CORE::hidp_send_message(struct hidp_session *session, struct socket *sock,
...
int HIDP_CORE::hidp_send_ctrl_message(struct hidp_session *session,
...
int HIDP_CORE::hidp_send_intr_message(struct hidp_session *session,
				  unsigned char hdr, const unsigned char *data,
				  int size)
{
...
int HIDP_CORE::hidp_input_event_cls(struct input_dev *dev, unsigned int type,
...
void HIDP_CORE::hidp_input_report(struct hidp_session *session, struct sk_buff *skb)
{
...
int HIDP_CORE::hidp_send_report(struct hidp_session *session, struct hid_report *report)
{
...
int HIDP_CORE::hidp_get_raw_report_cls(struct hid_device *hid,
...
int HIDP_CORE::hidp_output_raw_report_cls(struct hid_device *hid, unsigned char *data, size_t count,
		unsigned char report_type)
{
	struct hidp_session *session = hid->driver_data;
	int ret;
...
void HIDP_CORE::hidp_idle_timeout_cls(unsigned long arg)
{
...
void HIDP_CORE::hidp_set_timer(struct hidp_session *session)
{
	if (session->idle_to > 0)
		mod_timer(&session->timer, jiffies + HZ * session->idle_to);
}

void HIDP_CORE::hidp_del_timer(struct hidp_session *session)
{
	if (session->idle_to > 0)
		del_timer(&session->timer);
}

void HIDP_CORE::hidp_process_handshake(struct hidp_session *session,
					unsigned char param)
{
...
void HIDP_CORE::hidp_process_hid_control(struct hidp_session *session,
					unsigned char param)
{
	BT_DBG("session %p param 0x%02x", session, param);
...
int HIDP_CORE::hidp_process_data(struct hidp_session *session, struct sk_buff *skb,
				unsigned char param)
{
	int done_with_skb = 1;
...
void HIDP_CORE::hidp_recv_ctrl_frame(struct hidp_session *session,
					struct sk_buff *skb)
{
...
void HIDP_CORE::hidp_recv_intr_frame(struct hidp_session *session,
				struct sk_buff *skb)
{
	unsigned char hdr;
...
int HIDP_CORE::hidp_send_frame(struct socket *sock, unsigned char *data, int len)
{
...
void HIDP_CORE::hidp_process_transmit(struct hidp_session *session,
...
int HIDP_CORE::hidp_setup_input(struct hidp_session *session,
...
int HIDP_CORE::hidp_open(struct hid_device *hid)
{
	return 0;
}

void HIDP_CORE::hidp_close(struct hid_device *hid)
{
}

int HIDP_CORE::hidp_parse(struct hid_device *hid)
{
	struct hidp_session *session = hid->driver_data;
...
int HIDP_CORE::hidp_start(struct hid_device *hid)
{
	struct hidp_session *session = hid->driver_data;
	...
	list_for_each_entry(report, &hid->report_enum[HID_INPUT_REPORT].
			report_list, list)
		hidp_core.hidp_send_report(session, report);
...
void HIDP_CORE::hidp_stop(struct hid_device *hid)
{
	struct hidp_session *session = hid->driver_data;
...
int HIDP_CORE::hidp_setup_hid(struct hidp_session *session,
				struct hidp_connadd_req *req)
{
...
int HIDP_CORE::hidp_session_dev_init(struct hidp_session *session,
				 struct hidp_connadd_req *req)
{
	int ret;
...
void HIDP_CORE::hidp_session_dev_destroy(struct hidp_session *session)
{
	if (session->hid)
...
int HIDP_CORE::hidp_session_dev_add(struct hidp_session *session)
{
	int ret;
...
void HIDP_CORE::hidp_session_dev_del(struct hidp_session *session)
{
	if (session->hid)
		hid_destroy_device(session->hid);
	else if (session->input)
 ...
 */
int HIDP_CORE::hidp_session_new(struct hidp_session **out, const bdaddr_t *bdaddr,
...
void HIDP_CORE::hidp_session_get(struct hidp_session *session)
{
	kref_get(&session->ref);
}

/* release callback */
void HIDP_CORE::session_free_cls(struct kref *ref)
{
...
void HIDP_CORE::hidp_session_put(struct hidp_session *session)
{
 ...
 */
struct hidp_session* HIDP_CORE::__hidp_session_find(const bdaddr_t *bdaddr)
{
	struct hidp_session *session;
 ...
 */
struct hidp_session* HIDP_CORE::hidp_session_find(const bdaddr_t *bdaddr)
{
	struct hidp_session *session;
 ...
 */
int HIDP_CORE::hidp_session_start_sync(struct hidp_session *session)
{
 ...
 */
void HIDP_CORE::hidp_session_terminate(struct hidp_session *session)
{
 ...
 */
int HIDP_CORE::hidp_session_probe_cls(struct l2cap_conn *conn,
 ...
 */
void HIDP_CORE::hidp_session_remove_cls(struct l2cap_conn *conn,
				struct l2cap_user *user)
{
	struct hidp_session *session = container_of(user,
						    struct hidp_session,
 ...
 */
void HIDP_CORE::hidp_session_run(struct hidp_session *session)
{
 ...
 */
int HIDP_CORE::hidp_session_thread_cls(void *arg)
{
...
int HIDP_CORE::hidp_verify_sockets(struct socket *ctrl_sock,
			       struct socket *intr_sock)
{
	struct bt_sock *ctrl, *intr;
	struct hidp_session *session;
...
int HIDP_CORE::hidp_connection_add(struct hidp_connadd_req *req,
			struct socket *ctrl_sock,
			struct socket *intr_sock)
{
	struct hidp_session *session;
...
int HIDP_CORE::hidp_connection_del(struct hidp_conndel_req *req)
{
	struct hidp_session *session;
...
int HIDP_CORE::hidp_get_connlist(struct hidp_connlist_req *req)
{
	struct hidp_session *session;
	int err = 0, n = 0;
...
int HIDP_CORE::hidp_get_conninfo(struct hidp_conninfo *ci)
{
	struct hidp_session *session;
...
int hidp_connection_add(struct hidp_connadd_req *req,
				struct socket *ctrl_sock,
				struct socket *intr_sock){
	return hidp_core.hidp_connection_add(req, ctrl_sock, intr_sock);
}

int hidp_connection_del(struct hidp_conndel_req *req){
	return hidp_core.hidp_connection_del(req);
}
int hidp_get_connlist(struct hidp_connlist_req *req){
	return hidp_core.hidp_get_connlist(req);
}
int hidp_get_conninfo(struct hidp_conninfo *ci){
	return hidp_core.hidp_get_conninfo(ci);
}
	
static int hidp_open(struct hid_device *hid){
	return hidp_core.hidp_open(hid);
}
static void hidp_close(struct hid_device *hid){
	hidp_core.hidp_close(hid);
}
static int hidp_parse(struct hid_device *hid){
	return hidp_core.hidp_parse(hid);
}
static int hidp_start(struct hid_device *hid){
	return hidp_core.hidp_start(hid);
}
static void hidp_stop(struct hid_device *hid){
	hidp_core.hidp_stop(hid);
}

static int hidp_input_event(struct input_dev *dev, unsigned int type,
                             unsigned int code, int value) {
	return hidp_core.hidp_input_event_cls(dev, type, code, value);
}
int hidp_session_thread(void *arg){
	return hidp_core.hidp_session_thread_cls(arg);
}
void session_free(struct kref *ref){
	hidp_core.session_free_cls(ref);
}

void hidp_idle_timeout(unsigned long arg){
	hidp_core.hidp_idle_timeout_cls(arg);
}
        /* Probe HIDP session */
int hidp_session_probe(struct l2cap_conn *conn, struct l2cap_user *user){
	return hidp_core.hidp_session_probe_cls(conn, user);
}
        /* Remove HIDP session */
void hidp_session_remove(struct l2cap_conn *conn, struct l2cap_user *user){
	hidp_core.hidp_session_remove_cls(conn, user);
}
int hidp_get_raw_report(struct hid_device *hid,
                        unsigned char report_number,
                        unsigned char *data, size_t count,
                        unsigned char report_type){
	return hidp_core.hidp_get_raw_report_cls(hid, report_number, data, count, report_type);
}

int hidp_output_raw_report(struct hid_device *hid, unsigned char *data, size_t count,
                        unsigned char report_type){
	return hidp_core.hidp_output_raw_report_cls(hid, data, count, report_type);
}
